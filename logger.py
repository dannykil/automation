...
class LoggerFactory(object) :
    ...
    def create_logger() :
        ...
        # log 폴더 생성
        if (os.path.exists('./log/' + datetime.now().strftime('%Y%m')) == False) :
            os.makedirs('./log/' + datetime.now().strftime('%Y%m'))
        ...
        file_handler = logging.FileHandler('./log/' + datetime.now().strftime('%Y%m') + '/' + datetime.now().strftime('%Y%m%d') +'.log')
